...
function (documents, K, vocab, num.iterations, alpha, eta, initial = NULL, 
    burnin = NULL, compute.log.likelihood = FALSE, trace = 1L, save_state_interval = 0, odir="",
    freeze.topics = FALSE) 
{
    if (class(vocab) == "list") {
    ...
    retval <- structure(.Call("collapsedGibbsSampler", documents, 
        ...
        trace, as.integer(save_state_interval), toString(odir), as.logical(freeze.topics)), names = c("assignments", 
